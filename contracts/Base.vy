...
batchContract: public(address)
paused: public(bool)
admin: public(address)

@external
def __init__(_batchContract: address):
    self.paused = False
    self.admin = msg.sender
    self.batchContract = _batchContract
...
@external
@payable
def addLiquidityEthForUniV3(_tokenId: uint256, uniV3Params: MintParams, msg_sender: address) -> (address, uint256, address, uint256):
    assert msg.sender == self.batchContract
    ...
    if uniV3Params.token0 == WETH:
        if msg.value > uniV3Params.amount0Desired:
            send(msg_sender, msg.value - uniV3Params.amount0Desired)
        else:
            assert msg.value == uniV3Params.amount0Desired
        ...
        if uniV3Params.amount0Desired > amount0:
            WrappedEth(WETH).withdraw(uniV3Params.amount0Desired - amount0)
            send(msg_sender, uniV3Params.amount0Desired - amount0)
            ERC20(uniV3Params.token0).approve(NONFUNGIBLEPOSITIONMANAGER, 0)
        if uniV3Params.amount1Desired > amount1:
            ERC20(uniV3Params.token1).transfer(msg_sender, uniV3Params.amount1Desired - amount1)
        ...
        if msg.value > uniV3Params.amount1Desired:
            send(msg_sender, msg.value - uniV3Params.amount1Desired)
        else:
            assert msg.value == uniV3Params.amount1Desired
        ...
        (amount0, amount1, liquidity) = self.addLiquidity(_tokenId, msg_sender, uniV3Params)
        if uniV3Params.amount0Desired > amount0:
            ERC20(uniV3Params.token0).transfer(msg_sender, uniV3Params.amount0Desired - amount0)
            ERC20(uniV3Params.token0).approve(NONFUNGIBLEPOSITIONMANAGER, 0)
        if uniV3Params.amount1Desired > amount1:
            WrappedEth(WETH).withdraw(uniV3Params.amount1Desired - amount1)
            send(msg_sender, uniV3Params.amount1Desired - amount1)
            ERC20(uniV3Params.token1).approve(NONFUNGIBLEPOSITIONMANAGER, 0)
    return (uniV3Params.token0, amount0, uniV3Params.token1, amount1)

@external
def addLiquidityForUniV3(_tokenId: uint256, uniV3Params: MintParams, msg_sender: address) -> (address, uint256, address, uint256):
    assert msg.sender == self.batchContract
    assert not self.paused
    assert convert(uniV3Params.token0, uint256) < convert(uniV3Params.token1, uint256)
    ERC20(uniV3Params.token0).transferFrom(msg_sender, self, uniV3Params.amount0Desired)
...
@external
def removeLiquidityFromUniV3NFLP(_tokenId: uint256, _removeParams: RemoveParams) -> (address, uint256, address, uint256):
    assert msg.sender == self.batchContract
    assert _tokenId != 0
    ...
    amount0: uint256 = 0
    amount1: uint256 = 0
    (token0, token1, amount0, amount1) = self.removeLiquidity(_tokenId, _removeParams)
    return (token0, amount0, token1, amount1)

@external
def removeLiquidityEthFromUniV3NFLP(_tokenId: uint256, _removeParams: RemoveParams) -> (address, uint256, address, uint256):
    assert msg.sender == self.batchContract
    assert _tokenId != 0
    token0: address = ZERO_ADDRESS
    token1: address = ZERO_ADDRESS
...
@external
@payable
def investTokenForUniPair(_tokenId: uint256, _token: address, amount: uint256, _uniV3Params: SingleMintParams, msg_sender: address) -> (address, uint256, address, uint256):
    assert msg.sender == self.batchContract
    assert not self.paused
    assert amount > 0
    ...
    if token == VETH or token == ZERO_ADDRESS:
        if msg.value > amount:
            send(msg_sender, msg.value - amount)
        else:
...
@external
def divestUniV3NFLPToToken(_tokenId: uint256, _token: address, _removeParams: RemoveParams, minTokenAmount: uint256) -> (address, uint256, address, uint256):
    assert msg.sender == self.batchContract
...
@external
def newBatchContract(_batchContract: address):
    assert msg.sender == self.admin
    self.batchContract = _batchContract
